...
class ColumnSketchGenerator:
    ...
    wstate, initial_state, update_generator, ss_factor, dfactor, cfactor, prefix):
        ...
        self.usg = SeedGenerator("update_seed_pkg", "update_seeds", self.size, self.wuseed, prefix)
        self.dsg = DChainSignalGenerator(prefix)

        self.fpg = FunctionPackageGenerator(prefix)
    ...
    dfactor, cfactor, 1, 1, self.size, 1, prefix)
        self.muxg = MuxGenerator(prefix)
        self.mux_pkg = MuxConfigPackageGenerator(size, cfactor, self.wstate, prefix)

        self.demuxg = EnableDemuxGenerator(prefix)
        self.demux_pkg = EnableDemuxConfigPackageGenerator(size, dfactor, prefix)

        self.ssg = SingleStateSuperStageGenerator(self.wuseed, self.wvalue, self.wstate, self.initial_state, self.ug, self.dsg, prefix)
...
use WORK.{self.prefix}enable_demux_config_pkg.all;
"""
