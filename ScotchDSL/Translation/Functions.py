...
class FunctionGenerator:
    def create_entity_declarations(self):
        return "".join(map(lambda x: x.generateEntityDeclaration(self.enable), self.statements))

    def create_io_signals(self):
        return "".join(map(lambda x: x.generateInputOutputDeclarations(self.enable), self.statements))
    ...
    def create_port_declarations(self):
        return "".join(map(lambda x: x.generateComponentDeclaration(self.enable), self.statements))

    def create_port_mappings(self):
        return "".join(map(lambda x: x.generatePortMappings(self.enable), self.statements))

    def create_special_signal_connections(self):
        return "".join(map(lambda x: x.generateSpecialSignalAssignments(self.enable), self.statements))
    ...
    def generateArchitecture(self):
        ...
        return self.architecture_frame.format(self.entity_name, self.entity_name,
                                              signal_declarations + port_declarations,
                                              intermediate_connections + port_mappings + special_connections,
                                              self.entity_name)
...
"""
    ...
    def generateAssignmentFile(self):
        return "".join(map(lambda x: x.generateFullDefinition(self.synchronous, self.enable), self.statements))
    ...
    def dfs_distance(x):
        if isinstance(x, AstNodes.Assignment):
            ...
            vars = collectVectorVariables(x.tchild) + collectVectorVariables(x.fchild) + collectVectorVariables(
                x.condition)
        else:
        ...
        vars = list(filter(
            lambda x: isinstance(x.value, AstNodes.Assignment) or isinstance(x.value, AstNodes.ConditionalAssignment),
            vars))
        if not vars:
            return 1
        else:
...
class SelectorFunctionGenerator(FunctionGenerator):
    def __init__(self, statements, value_width, seed_width, offset_width, synchronous, enable=False, entity_name="selector", prefix=""):
        self.prefix=prefix
        ...
        self.entity_name = F"{prefix}{entity_name}"
        self.enable = enable
...
class UpdateFunctionGenerator(FunctionGenerator):
    def __init__(self, statements, value_width, seed_width, state_width, synchronous, prefix=""):
        self.prefix = prefix
        self.statements = statements
        self.value_width = value_width
        self.seed_width = seed_width
        ...
        self.entity_name = F"{prefix}update"
        self.signals = self.generateDeclarationSignals()
        self.enable = False
...
class DataParallelUpdateFunctionGenerator(UpdateFunctionGenerator):
    ...
    def __init__(self, statements, value_width, seed_width, state_width, n_parallel, synchronous, prefix=""):
        self.n_parallel = n_parallel
        super().__init__(statements, value_width, seed_width, state_width, synchronous, prefix=prefix)
