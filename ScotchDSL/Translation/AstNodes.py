...
class GeneralAssignment(AstNode):
    ...
    def __init__(self, prefix=""):
        #Is overriden by an inheriting class
        self.prefix = prefix
...
"""
    ...
    def getNodeName(self):
        return F"{self.prefix}as{id(self)}"

    def generateFullDefinition(self, synchronous=True, enable=False):
        return self.generateIncludes() + self.generateEntityDeclaration(enable) + self.generateArchitectureDefinition(
            synchronous, enable)

    # We need an output signal for every assignment.
    def generateCommonInputOutputDeclarations(self, enable=False):
        node_prefix = self.getNodeName()
        signal_list = []
        signal_list.append(
            "signal {}_{}_out : {}".format(node_prefix, self.lchild.get_full_name(), self.lchild.getVHDLType()))
        signal_list.append("signal {}_{}_in : {}".format(node_prefix, "clk", "STD_LOGIC"))
        if enable:
            signal_list.append("signal {}_{}_in : {}".format(node_prefix, "enable", "STD_LOGIC"))

        return ";\n".join(signal_list) + ";\n"

    def generateSpecialSignalAssignments(self, enable=False):
        node_prefix = self.getNodeName()
        signal_list = []

        if enable:
            signal_list.append("{}_enable_in <= enable".format(node_prefix))
    ...
    def generateCommonMappings(self, enable=False):
        node_prefix = self.getNodeName()
        signal_list = []

        signal_list.append("{} => {}_{}_in".format("clk", node_prefix, "clk"))
        if enable:
            signal_list.append("{} => {}_{}_in".format("enable", node_prefix, "enable"))
        return ",\n".join(signal_list) + ",\n"

    def generateEntityDeclaration(self, enable=False):
        frame = """
ENTITY {} IS
PORT (
{}
...
END {};"""
        ...
        port_block = self.generatePorts(enable)
        return frame.format(nname, port_block, nname)

    def generateComponentDeclaration(self, enable):
        frame = """
COMPONENT {} IS
PORT (
{}
...
END COMPONENT;"""
    ...
    def generatePortMappings(self, enable=False):
        frame = """
{}_instance: {}
PORT MAP(
{}
...
"""
        ...
        nname = self.getNodeName()
        port_assignments = self.generateCommonMappings(enable) + self.generatePortAssignments()
    ...
    def generateIntermediateConnections(self):
        ...
        for v in in_variables:
            if isinstance(v.value, GeneralAssignment):
                signal_set.add("{}_{}_in <= {}_{}_out".format(node_prefix, v.get_full_name(), v.value.getNodeName(),
                                                              v.get_full_name()))
    ...
    def generateArchitectureDefinition(self, synchronous=True, enable=False):
        # Computation of outstate is always unpipelined
        assert not (synchronous == False and enable == True)
        if self.lchild.name == "outstate":
            synchronous = False
        asynchronous_frame = """
...
class ConditionalAssignment(GeneralAssignment):
    def __init__(self, condition, lchild, tchild, fchild, tchild_arithmetic, fchild_arithmetic, prefix=""):
...
"""
        ...
        if self.tchild_arithmetic:
            if not type_visitor.type is None:
                tstatement = "std_logic_vector(to_{}({}, {}))".format(type_visitor.type, self.tchild.toVHDL(),
                                                                      self.lchild.width)
            else:
        ...
        if self.fchild_arithmetic:
            if not type_visitor.type is None:
                fstatement = "std_logic_vector(to_{}({}, {}))".format(type_visitor.type, self.fchild.toVHDL(),
                                                                      self.lchild.width)
            else:
                # If there was no arithmetic conversion operator, it's probably just some plain arithmetic.
                # TODO: In the future, we should do better than crossing fingers here.
        ...
        if synchronous:
            assignment = frame_synchronous.format(self.condition.toVHDL(), self.lchild.toVHDL(), tstatement,
                                                  self.lchild.toVHDL(), fstatement)
        else:
            assignment = frame_asynchronous.format(self.lchild.toVHDL(), tstatement, self.condition.toVHDL(),
                                                   fstatement)
    ...
    def generateInputOutputDeclarations(self, enable=False):
        node_prefix = self.getNodeName()

        commons = self.generateCommonInputOutputDeclarations(enable)
    ...
    def generatePorts(self, enable=False):
        signals = list(GeneralAssignment.common_signals)
        if enable:
            signals.append("enable : in std_logic")
...
class Assignment(GeneralAssignment):
    def __init__(self, lchild, rchild, arithmetic, prefix=""):
        super().__init__(prefix)
        self.lchild = lchild
        self.rchild = rchild
    ...
    def generateVHDLSignalAssignment(self, synchronus=True):
        if not self.arithmetic:
            ...
            if not type_visitor.type is None:
                assignment = "{} <= std_logic_vector(to_{}({}, {}));".format(self.lchild.toVHDL(), type_visitor.type,
                                                                             self.rchild.toVHDL(), self.lchild.width)
            else:
                # If there was no arithmetic conversion operator, it's probably just some plain arithmetic.
                # TODO: In the future, we should do better than crossing fingers here.
                assignment = "{} <= std_logic_vector(to_signed({}, {}));".format(self.lchild.toVHDL(),
                                                                                 self.rchild.toVHDL(),
                                                                                 self.lchild.width)
...
class InvertOperation(UnaryAstNode):
    def toVHDL(self):
        return "(not ({}))".format(self.child.toVHDL())

class ConcatOperation(BinaryAstNode):
    def toVHDL(self): 
        return "({} & {})".format(self.lchild.toVHDL(), self.rchild.toVHDL())

class MinMaxOperation(BinaryAstNode):
    def toVHDL(self): 
        return "minmax({},{})".format(self.lchild.toVHDL(), self.rchild.toVHDL())

class MaxMaxOperation(BinaryAstNode):
    def toVHDL(self): 
        return "maxmax({},{})".format(self.lchild.toVHDL(), self.rchild.toVHDL())
